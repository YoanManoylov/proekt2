...
def choose_mode():
    ...
    while True:
        ...
        print("\u26A0\uFE0F Грешен избор, пробвай отново")
...
def play_game(max_number, max_attempts, start_time):
    ...
    messages = deque(["Можеш! \U0001F4AA", "Не се отказвай \U0001F680", "Почти си там \U0001F3AF"])
    print(f"\nПознайте число между 1 и {max_number}. Имате {max_attempts} опита.")
    print("Командите са:'история','изход','подсказка'")
    ...
    print(f"\u274C Не успя. Числото е {secret}.")
    return False, 0
...
def main():
    ...
    while True:
        ...
        if max_attempts > 3:
            max_attempts -= 1
        print(f"\n\U0001F680 Ниво нагоре, нов диапазон: 1-{max_number}, брои опити: {max_attempts}")
